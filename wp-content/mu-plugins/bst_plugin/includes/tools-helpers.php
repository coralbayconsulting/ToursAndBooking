...
/**
 * Single “latest” saved log for Tools: compares vehicle migration vs release cleanup options by time.
 *
 * @return array{ time: string, text: string, source: string }|null
 */
function bst_tools_get_latest_release_migration_log() {
    $mig = get_option( 'bst_migration_last_run', null );
    $rel = get_option( 'bst_last_release_cleanup_log', null );

    $candidates = array();

    if ( is_array( $mig ) && ! empty( $mig['text'] ) ) {
        $ts = 0.0;
        if ( isset( $mig['utc'] ) && is_numeric( $mig['utc'] ) ) {
            $ts = (float) $mig['utc'];
        } elseif ( ! empty( $mig['time'] ) ) {
            $t = strtotime( $mig['time'] );
            $ts = false !== $t ? (float) $t : 0.0;
        }
        $candidates[] = array(
            'ts'     => $ts,
            'time'   => isset( $mig['time'] ) ? (string) $mig['time'] : '',
            'text'   => (string) $mig['text'],
            'source' => 'vehicle migration',
        );
    }

    if ( is_array( $rel ) && ! empty( $rel['text'] ) ) {
        $ts = 0.0;
        if ( ! empty( $rel['time'] ) ) {
            $t = strtotime( $rel['time'] );
            $ts = false !== $t ? (float) $t : 0.0;
        }
        $candidates[] = array(
            'ts'     => $ts,
            'time'   => isset( $rel['time'] ) ? (string) $rel['time'] : '',
            'text'   => (string) $rel['text'],
    ...
    usort(
        ...
        function ( $a, $b ) {
            ...
            return ( 'release cleanup' === $a['source'] ) ? -1 : 1;
        }
    );
