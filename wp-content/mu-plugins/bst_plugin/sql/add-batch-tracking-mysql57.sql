-- BST Plugin: Add Email Batch Tracking (MySQL 5.7 Compatible)
-- For older MySQL versions that don't support "IF NOT EXISTS" in ALTER TABLE
-- Replace 'wp_' with your actual WordPress table prefix if different

-- Add batch_id column to email_log table
-- If column already exists, this will produce an error that can be safely ignored
ALTER TABLE wp_bst_email_log 
ADD COLUMN batch_id BIGINT UNSIGNED NULL AFTER message_id;

-- Add index for batch_id
ALTER TABLE wp_bst_email_log 
ADD INDEX idx_batch_id (batch_id);

-- Add error_message column to email_log table
ALTER TABLE wp_bst_email_log 
ADD COLUMN error_message TEXT NULL AFTER batch_id;
